...
plan_ranker::CandidatePlan MultiPlanner::finalizeExecutionPlans(
    ...
    auto& stats = std::get<std::vector<std::unique_ptr<PlanStageStats>>>(decision->stats);
    ...
    LOGV2_DEBUG(2059101,
                ...
                "Explain_getPlanSummary_bestCandidate_root"_attr =
                    Explain::getPlanSummary(winner.root.get()));
    ...
    // and, as a result, we cannot stash the results returned so far in the plan executor.
    if (!stats[winnerIdx]->common.isEOF &&
        (winner.solution->hasNode(STAGE_SORT_DEFAULT) ||
         winner.solution->hasNode(STAGE_SORT_SIMPLE))) {
        winner.root->close();
    ...
    updatePlanCache(_opCtx, _collection, _cachingMode, _cq, std::move(decision), candidates);
