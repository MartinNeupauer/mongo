...
plan_ranker::CandidatePlan CachedSolutionPlanner::finalizeExecutionPlan(
    ...
    auto ranker{plan_ranker::makePlanRanker(candidate.solution.get())};
    updatePlanCacheFeedback(_collection, _cq, ranker->calculateRank(stats.get()));
    ...
    if (!stats->common.isEOF &&
        (candidate.solution->hasNode(STAGE_SORT_DEFAULT) ||
         candidate.solution->hasNode(STAGE_SORT_SIMPLE))) {
