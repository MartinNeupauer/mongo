...
std::unique_ptr<PlanStage> ClassicStageBuilder::build(const QuerySolutionNode* root) {
    auto* const expCtx = _cq.getExpCtx().get();
    ...
    switch (root->getType()) {
        ...
        case STAGE_IXSCAN: {
            ...
            return std::make_unique<IndexScan>(expCtx, std::move(params), _ws, ixn->filter.get());
        }
        case STAGE_FETCH: {
        ...
        case STAGE_PROJECTION_COVERED: {
            ...
            auto childStage = build(pn->children[0]);
            return std::make_unique<ProjectionStageCovered>(_cq.getExpCtx().get(),
                                                            _cq.getQueryRequest().getProj(),
                                                            _cq.getProj(),
                                                            _ws,
                                                            std::move(childStage),
        ...
        case STAGE_PROJECTION_SIMPLE: {
            ...
            auto childStage = build(pn->children[0]);
            return std::make_unique<ProjectionStageSimple>(_cq.getExpCtx().get(),
                                                           _cq.getQueryRequest().getProj(),
                                                           _cq.getProj(),
                                                           _ws,
                                                           std::move(childStage));
        ...
        case STAGE_GEO_NEAR_2D: {
            ...
            return std::make_unique<GeoNear2DStage>(params, expCtx, _ws, twoDIndex);
        }
        case STAGE_GEO_NEAR_2DSPHERE: {
            ...
            return std::make_unique<GeoNear2DSphereStage>(params, expCtx, _ws, s2Index);
        }
        case STAGE_TEXT: {
            ...
            return std::make_unique<TextStage>(expCtx, params, _ws, node->filter.get());
        }
        ...
        case STAGE_DISTINCT_SCAN: {
            ...
            return std::make_unique<DistinctScan>(expCtx, std::move(params), _ws);
        }
        case STAGE_COUNT_SCAN: {
            ...
            return std::make_unique<CountScan>(expCtx, std::move(params), _ws);
        }
