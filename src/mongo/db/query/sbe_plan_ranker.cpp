 ...
 */
class DefaultPlanRanker final : public mongo::plan_ranker::BasePlanRanker<PlanStageStats> {
public:
    DefaultPlanRanker(const QuerySolution* solution) : _solution{solution} {
        invariant(_solution);
    }

protected:
    std::pair<double, std::string> calculateProductivity(
        const mongo::sbe::PlanStageStats* root) const final {
        auto numReads{calculateNumberOfReads(root)};
        auto productivity = static_cast<double>(root->common.advances) /
    ...
    double calculateTieBreakerEpsilon(const mongo::sbe::PlanStageStats* stats) const final {
        return std::min(
            1.0 /
                static_cast<double>(10 * (stats->common.advances > 0 ? stats->common.advances : 1)),
            1e-4);
    }
...
std::unique_ptr<mongo::plan_ranker::PlanRanker<PlanStageStats>> makePlanRanker(
    ...
    return std::make_unique<DefaultPlanRanker>(solution);
}
}  // namespace mongo::sbe::plan_ranker
