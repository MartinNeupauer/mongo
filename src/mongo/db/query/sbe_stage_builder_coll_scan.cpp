...
generateOptimizedOplogScan(OperationContext* opCtx,
    ...
    if (csn->filter) {
        ...
        //              <stage>
        //           right
        //              ixseek seekRecordIdSlot resultSlot recordIdSlot @coll
        //
...
generateGenericCollScan(const Collection* collection,
    ...
    const auto canUseParallelScan =
        !csn->resumeAfterRecordId && forward && internalQueryDefaultDOP.load() > 1;
...
generateCollScan(OperationContext* opCtx,
    ...
    auto [resultSlot, recordIdSlot, oplogTsSlot, stage] = [&]() {
        if (csn->minTs || csn->maxTs) {
            return generateOptimizedOplogScan(opCtx, collection, csn, slotIdGenerator, yieldPolicy);
        } else {
            return generateGenericCollScan(collection, csn, slotIdGenerator, yieldPolicy);
        }
    }();
    ...
    if (csn->direction == CollectionScanParams::FORWARD && localDop > 1) {
        ...
        stage = sbe::makeS<sbe::ExchangeConsumer>(
            std::move(stage), localDop, slots, sbe::ExchangePolicy::roundrobin, nullptr, nullptr);
    }
