...
std::vector<std::pair<BSONObj, BSONObj>> decomposeIntoSingleIntervals(
    ...
    auto appendInterval = [lowKeyInclusive, highKeyInclusive](const BSONObj& lowKey,
        ...
        if (interval.isMinToMax() || interval.isMaxToMin()) {
            IndexBoundsBuilder::appendAllValuesInterval(
...
makeIntervalsFromIndexBounds(const IndexBounds& bounds,
    ...
    LOGV2_DEBUG(47429005,
                5,
                "Generated {intervals} interval(s) for ixscan...",
                "intervals"_attr = intervals.size());
    std::vector<std::pair<std::unique_ptr<KeyString::Value>, std::unique_ptr<KeyString::Value>>>
        result;
 ...
 *
 * 1. The anchor union branch is the starting point of the recursive subtree. It pushes the
 *    starting index into the lspool stage. The lspool has a filter predicate to ensure that
 *    only index keys will be stored in the pool.
 * 2. There is a filter stage at the top of the subtree to ensure that we will only produce
 *    recordId values.
 * 3. The recursive union branch does the remaining job. It has a nest loop join with the outer
 *    branch being a stack spool, which reads data from the lspool above.
 *        3.1 The outer branch reads next seek key from sspool.
 *        3.2 The seek key is passed to the inner branch.
 *        3.3 The inner branch execution starts with the projection of the seek key, which is
 *            fed into the ixscan as a 'lowKeySlot'.
 *        3.4 Two slots produced by the ixscan, 'resultSlot' and 'recordIdSlot', are passed to
 *            the chkbounds stage. Note that 'resultSlot' would contain the index key.
 *        3.5 The chkbounds stage can produce one of the following values:
 *               * The recordId value, taken from the ixscan stage, if the index key is within the
 *                 bounds.
 *               * An EOF if the key is past the bounds.
 ...
 *        3.6 If chkbounds returns EOF, the process repeats from 3.1.
 *        3.7 Otherwise, the produces values is pulled up to the lspool stage and either is
 *            stored in the buffer, if it was a seek key, or just propagated to the upper stage as a
 *            valid recordId, and the process continues from 3.4 by fetching the next key from the
 *            index.
 * 4. The recursion is terminated when the sspool becomes empty.
 */
std::pair<sbe::value::SlotId, std::unique_ptr<sbe::PlanStage>>
