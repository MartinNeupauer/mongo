...
/**
 * Stores the best candidate plan, chosen from the given 'candidates' based on the 'ranking'
 * decision, in the plan cache, if the 'query' is of a type that is safe to cache.
 *
 * The 'cachingMode' specified the caching mode, that is, whether the query should be always
 * cached, never, or sometimes when we hit some special cases.
 */
...
/**
 * Updates the plan cache entry for the given 'query' by adding the 'score' to the cache's entry
 * 'feedback' array.
 */
