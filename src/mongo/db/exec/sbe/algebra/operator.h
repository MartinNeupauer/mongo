...
#pragma once
#include "mongo/db/exec/sbe/algebra/polyvalue.h"
#include <iostream>
#include <vector>
...
class OpWalker {
    ...
    struct Deducer<T, true> {
        using type = decltype(
            std::declval<D>().walk(std::declval<T>(), std::declval<detail::OpConcreteType<T>&>()));
    };
    template <typename T>
    struct Deducer<T, false> {
        using type = decltype(std::declval<D>().walk(std::declval<detail::OpConcreteType<T>&>()));
    };
    template <typename T>
    using deduced_t = typename Deducer<T, withSlot>::type;
    ...
    template <typename N, typename T, size_t... I>
    auto walkUnpack(N&& slot, T&& op, std::index_sequence<I...>) {
        return walkStep(std::forward<N>(slot), std::forward<T>(op), op.template get<I>()...);
    }
    template <typename N, typename T, size_t... I>
    auto walkDynamicUnpack(N&& slot, T&& op, std::index_sequence<I...>) {
        return walkStep(
            std::forward<N>(slot), std::forward<T>(op), op.nodes(), op.template get<I>()...);
    }
...
template <bool withSlot = false, typename D, typename N>
auto walk(N&& node, D& domain) {
    return node.visit(OpWalker<D, withSlot>{domain});
}
}  // namespace algebra
}  // namespace sbe
}  // namespace mongo