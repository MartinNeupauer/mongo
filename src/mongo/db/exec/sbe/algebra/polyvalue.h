...
public:
    ...
    template <typename V>
    static auto visit(V&& v, PolyValueType& holder, AbstractType* block) {
        return v(holder, *cast<T>(block));
    }
    template <typename V>
    static auto visitConst(V&& v, const PolyValueType& holder, const AbstractType* block) {
        return v(holder, *castConst<T>(block));
    }
};
...
public:
    ...
    auto visit(V&& v) {
        ...
        using FunPtrType = decltype(&ControlBlockVTable<get_t<0>, Ts...>::template visit<V>);
        static constexpr FunPtrType visitTbl[] = {
            &ControlBlockVTable<Ts, Ts...>::template visit<V>...};

        check();
        return visitTbl[tag()](std::forward<V>(v), *this, _object);
    }
    ...
    auto visit(V&& v) const {
        ...
        using FunPtrType = decltype(&ControlBlockVTable<get_t<0>, Ts...>::template visitConst<V>);
        static constexpr FunPtrType visitTbl[] = {
            &ControlBlockVTable<Ts, Ts...>::template visitConst<V>...};
